...
graph TD
    ...
    G --> H[注册 API 处理器]
    H --> I[headscale 命令执行]
    I --> J[headscale nodes register<br/>--user flink --key XXX]
...
sequenceDiagram
    ...
    API->>Register: 处理注册请求
    Register->>Headscale: headscale nodes register --user flink --key XXX
...
带认证访问：curl -x http://user:D5D0R2gwvodpIKhEyZ4lrFVO@localhost:8081 https://example.com → 直接路由到 key 对应的特定节点
  ```
  # 带认证 - 指定节点
curl -x http://any:D5D0R2gwvodpIKhEyZ4lrFVO@localhost:8081 https://example.com

# SOCKS5 也支持认证
curl --socks5 any:D5D0R2gwvodpIKhEyZ4lrFVO@localhost:1080 https://example.com
  ```
这样就能实现基于 key 的精确节点路由，同时保持向后兼容。

> 认证方式说明：
> 代理认证仅支持标准 URL 认证（user:key@host:port）。